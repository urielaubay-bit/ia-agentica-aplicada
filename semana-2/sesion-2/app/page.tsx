...
export default function Page() {
  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: 16 }}>
      <h2>Mi agente (UI generativa)</h2>
      {messages.map((m) => (
        <div key={m.id} style={{ marginBottom: 12 }}>
          <b>{m.role === "user" ? "Tu" : "Asistente"}:</b>
          {m.parts.map((part, i) => {
            if (part.type === "text") return <p key={i}>{part.text}</p>;
            if (part.type === "tool-getKpiSummary" && part.state === "output-available") {
              const d = part.output as any;
              return <KpiCards key={i} kpis={d.kpis} caption={`${d.channel} · ${d.range}`} />;
            }
            return null;
          })}
        </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (input.trim()) { sendMessage({ text: input }); setInput(""); } }}>
        <input style={{ width: "100%", padding: 8 }} value={input}
          ...
          placeholder="Muestrame los KPIs de los ultimos 30 dias" />
      </form>
